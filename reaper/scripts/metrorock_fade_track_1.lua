-- Smoothly fades track 1 between 0 dB and silence over 2 seconds.
...
local function tick()
    ...
    if t >= 1 then
        reaper.SetMediaTrackInfo_Value(track, "D_VOL", db_to_linear(target_db))
        return
    end
    local cur_db_now = start_db + (target_db - start_db) * t
    reaper.SetMediaTrackInfo_Value(track, "D_VOL", db_to_linear(cur_db_now))
    reaper.defer(tick)
end
